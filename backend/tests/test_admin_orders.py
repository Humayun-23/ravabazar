...
def db_session():
    ...
    # We only need Admin and Order for these tests
...
def test_order(db_session):
    ...
    order = Order(
        user_id=user.id,
        address_snapshot={"city": "Test City"},
        total_amount=100.0,
        final_amount=100.0,
...
def test_admin_orders_rbac(client: TestClient, test_customer_token):
    headers = {"Authorization": f"Bearer {test_customer_token}"}
    res = client.get("/api/v1/admin/orders", headers=headers)
    assert res.status_code == 403
    
    res = client.get("/api/v1/admin/orders/1", headers=headers)
    assert res.status_code == 403
    
    res = client.patch("/api/v1/admin/orders/1/status", json={"status": "processing"}, headers=headers)
    assert res.status_code == 403

def test_admin_list_orders(client: TestClient, test_admin_token, test_order):
    headers = {"Authorization": f"Bearer {test_admin_token}"}
    res = client.get("/api/v1/admin/orders", headers=headers)
    assert res.status_code == 200
    data = res.json()
    assert data["total"] == 1
    assert data["items"][0]["id"] == test_order.id

def test_admin_get_order(client: TestClient, test_admin_token, test_order):
    headers = {"Authorization": f"Bearer {test_admin_token}"}
    res = client.get(f"/api/v1/admin/orders/{test_order.id}", headers=headers)
    assert res.status_code == 200
    data = res.json()
